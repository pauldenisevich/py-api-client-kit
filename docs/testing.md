...
* `DecodeError` hierarchy and its independence from `HttpStatusError` and
  `ValueError`
* required package `ResponseData`, response identity, explicit
  `error.response.raw` access, and response exclusion from `Exception.args`
